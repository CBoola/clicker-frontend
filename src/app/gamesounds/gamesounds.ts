...
export class GamesoundsService {
  ...
  achievementsSounds = 'wygranko.mp3';

  constructor(public gs: GameState) {
    this.gs.structuresUpdated.subscribe(res => {
      this.loadStructuresSounds();
    });
    this.gs.upgradesUpdated.subscribe(res => {
      this.loadUpgradesSounds();
    });
    new Audio(this.prefix + this.achievementsSounds + '');
  }

  loadStructuresSounds() {
    for (const structure of this.gs.structures) {
  ...
  loadUpgradesSounds() {
    for (const upgrade of this.gs.upgrades) {
